...
- (void)endRefresh
{
    ...
    if (self.refreshAnimationDisabled){
        [self resetTopEdge];
    }
    else{
...
- (void)checkContentOffset:(CGPoint)contentOffset
{
    ...
    CGRect frame = self.tableViewRefreshView.frame;
    frame.size.width = self.bounds.size.width;
    self.tableViewRefreshView.frame = frame;
...
- (BOOL)refreshAnimationDisabled{
    return [objc_getAssociatedObject(self, &@selector(refreshAnimationDisabled)) boolValue];
}
- (void)setRefreshAnimationDisabled:(BOOL)refreshAnimationDisabled{
    objc_setAssociatedObject(self, &@selector(refreshAnimationDisabled), @(refreshAnimationDisabled), OBJC_ASSOCIATION_RETAIN);
}
